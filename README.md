...
> **Beta branch.** These materials install `v0.7.0-beta.10`, a pre-release build of the
...
docker run -d --name mcp-gateway --network mcp-gateway --restart unless-stopped \
  ...
  dxheroes/mcp-gateway-ce:v0.7.0-beta.10
```
...
To use GHCR, add `GATEWAY_IMAGE=ghcr.io/dxheroes/mcp-gateway-ce:v0.7.0-beta.10` to `.env` (Compose) or
