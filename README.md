...
> **Beta branch.** These materials install `v1.2.1-beta.4`, a pre-release build of the
...
docker run -d --name mcp-gateway --network mcp-gateway --restart unless-stopped \
  ...
  dxheroes/mcp-gateway-ce:v1.2.1-beta.4
```
...
To use GHCR, add `GATEWAY_IMAGE=ghcr.io/dxheroes/mcp-gateway-ce:v1.2.1-beta.4` to `.env` (Compose) or
