...
> **Beta branch.** These materials install `v1.2.0-beta.9`, a pre-release build of the
...
docker run -d --name mcp-gateway --network mcp-gateway --restart unless-stopped \
  ...
  dxheroes/mcp-gateway-ce:v1.2.0-beta.9
```
...
To use GHCR, add `GATEWAY_IMAGE=ghcr.io/dxheroes/mcp-gateway-ce:v1.2.0-beta.9` to `.env` (Compose) or
